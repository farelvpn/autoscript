...
def print_json_response(data, success=True, code=200):
    ...
    if success:
        response["message"] = f"Kuota untuk user VMess '{data.get('username')}' berhasil ditambahkan"
        response["data"] = data
    else:
...
def add_vmess_user_quota(params):
    """Fungsi utama untuk menambahkan kuota ke user VMess."""
