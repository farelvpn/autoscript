...
def print_json_response(data, success=True, code=200):
    ...
    if success:
        response["message"] = f"Akun '{data.get('username')}' berhasil dihapus permanen"
        response["data"] = data
    else:
...
def send_telegram_notification(username):
    ...
    try:
        ...
        text = (
            f"<b>🗑️ VMESS ACCOUNT DELETED (PERMANENT)</b>\n"
            f"━━━━━━━━━━━━━━━━━━\n"
...
def delete_vmess_account(params):
    """Fungsi utama untuk menghapus akun VMESS secara permanen."""
    ...
    db_path = os.path.join(DIR_DATABASE_VMESS, f"{username}.txt")
    if not os.path.exists(db_path):
        print_json_response({"message": f"User VMESS '{username}' tidak ditemukan."}, success=False, code=404)

    # 2. Baca Info User (untuk respons) & Hapus File Database
    ...
    # 3. Hapus File-file Terkait Lainnya
    files_removed = ["database"]
    quota_path = os.path.join(DIR_QUOTA_VMESS, username)
    if os.path.exists(quota_path):
        os.remove(quota_path)
        files_removed.append("quota_limit")
    
    # 4. Hapus User dari Konfigurasi
