...
DIR_USAGE = '/etc/xray/usage/quota/vless'
...
def print_json_response(data, success=True, code=200):
    ...
    if success:
        response["message"] = f"Detail untuk user VLESS '{data.get('username')}' berhasil diambil"
        response["data"] = data
    else:
...
def parse_access_log(username):
    ...
    try:
        # Regex untuk mencocokkan log akses
...
def get_vless_user_info(params):
    """Fungsi utama untuk mendapatkan detail user VLESS."""
