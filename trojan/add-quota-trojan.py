...
# --- Konfigurasi Path ---
DIR_QUOTA_LIMIT = '/etc/xray/limit/quota/trojan'
...
def print_json_response(data, success=True, code=200):
    ...
    if success:
        response["message"] = f"Kuota untuk user '{data.get('username')}' berhasil ditambahkan"
        response["data"] = data
    else:
...
def send_telegram_notification(username, quota_added_gb, new_total_quota_display):
    ...
    try:
        ...
        if not bot_token or not chat_id:
            return # Lewati jika token atau ID kosong

        message = (
            f"<b>✅ KUOTA DITAMBAHKAN</b>\n"
            f"━━━━━━━━━━━━━━━━━━\n"
        ...
        # Gunakan curl via subprocess agar tidak ada dependensi eksternal
        subprocess.run(
        ...
        # Kegagalan mengirim notifikasi tidak menghentikan proses utama
        # Cukup catat di log stderr server jika terjadi error
        print(f"WARNING: Gagal mengirim notifikasi Telegram: {e}", file=sys.stderr)

# --- Logika Utama ---

def add_user_quota(params):
    """Fungsi utama untuk menambahkan kuota ke user."""
    ...
    response_data = {
        ...
        "telegram_notification": "sent" # Menambahkan status notifikasi
    }
