...
DIR_QUOTA_USAGE = '/etc/xray/usage/quota/trojan'
...
def send_telegram_notification(username):
    ...
    try:
        ...
        message = (
            f"<b>AKUN DIHAPUS (QUOTA HABIS)</b>\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"<b>Username : </b> <code>{username}</code>\n"
            f"<b>Alasan   : </b> Kuota data telah terlampaui."
        )

        # Menggunakan subprocess untuk memanggil curl agar tidak butuh library eksternal
        subprocess.run(
...
def delete_user_permanently(username):
    ...
    print(f"INFO: Kuota terlampaui untuk '{username}'. Memulai penghapusan permanen.")
    ...
    try:
        ...
        if user_found:
            ...
            # Panggil fungsi notifikasi setelah user berhasil dihapus
...
def main():
    ...
    users = get_all_users()
    if not users:
        print("INFO: Tidak ada user yang ditemukan untuk dicek.")
        return
